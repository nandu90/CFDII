...
int main()
{
    ...
    /*****Read Control File*****/    
    control(x0,xn,nelem,u,degree,cfl,a,fluxscheme,totaltime,stages,ucenter,reconstruct);
    /****************************/
    ...
    while (t<totaltime)
    {
        if (stages == 1)
        {
        rhsvector(R, nelem, uelem, degree, a, fluxscheme, deltax,reconstruct);
    ...
    vector< vector <double> > unew(nelem, vector<double> (degree,0));
    ...
    else if(degree == 2)
    {
        ...
        for(int i=1;i<nelem-1;i++)
        {
            vector<double> Uh(degree,0);
                Uh[0]=uelem[i][0]-uelem[i][1];
                Uh[1]=uelem[i][0]+uelem[i][1];
    ...
    else if(degree == 3)
    {
        ...
        {
            vector<double> Uh(degree,0);
                Uh[0]=uelem[i][0]-uelem[i][1]+uelem[i][2];
                Uh[1]=uelem[i][0]-uelem[i][2]/2;
                Uh[2]=uelem[i][0]+uelem[i][1]+uelem[i][2];
            for(int j=0;j<degree;j++)
            {
                unew[i][j]=Uh[j];
            }
    ...
    else if(degree == 3)
    {
        for(int i=1;i<nelem-1;i++)
        {
            ...
            if(k == 0){output<<x;}
                ...
                if (k == 0)
                {
                    output<<" "<<unew[i][k]<<" "<<ui[i];
                }
                else if (k==1)
                {
                    output<<" "<<unew[i][k]<<" "<<ucenter[i];
                }
                else if (k==2)
                {
                    output<<" "<<unew[i][k]<<" "<<ui[i+1];
                }
